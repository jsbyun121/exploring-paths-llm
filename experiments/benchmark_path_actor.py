...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("outputs/analysis/actor_benchmark.json"))
    parser.add_argument("--repeats", type=int, default=3)
    args = parser.parse_args()
    initialize_global_process_group()
    cfg = OmegaConf.load("outputs/2026-09-03/15-47-58/.hydra/config.yaml").actor
    cfg.path.causal_padding_positions = False
    ...
    for lengths in [(640, 1280), (2048, 3200)]:
        ...
        reference = None
        for mode in ["padded", "trimmed", "trimmed_causal"]:
            cfg.path.causal_padding_positions = mode == "trimmed_causal"
            current = batch if mode == "padded" else trim_right_padding(batch)
            times = []
            for repeat in range(args.repeats + 1):
                ...
                losses, entropy, _, _ = actor.forward_path(current)
                loss = (losses.sum(-1) / current["action_mask"].sum(-1)).mean()
                loss.backward()
                torch.cuda.synchronize()
