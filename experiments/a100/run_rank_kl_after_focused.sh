#!/usr/bin/env bash
# Queue Rank-KL only after successful completion of the focused diagnostic.
...
printf 'Waiting for the current focused experiment to release the GPU lock.\n'
flock 9
python - <<'CHECK'
import json, os
from pathlib import Path
root = Path(os.environ["FOCUSED_ROOT"])
p = root / "ce100_vs_rank100.json"
a = json.loads(p.read_text())
assert a['examples'] == 200, 'Expected completed matched 200-example comparison'
for name in ['positive_ce_step100', 'rank_jsd_step100']:
    a = json.loads((root / 'evaluation' / name / 'summary.json').read_text())
    assert a['examples'] == 200 and a['sample_k'] == 8
print('Previous CE/Rank-JSD evaluation completed; starting Rank-KL.')
CHECK
...
export MAX_STEPS=100 TEST_FREQ=25 SAVE_FREQ=10 KEEP_CHECKPOINTS=2 METHODS=rank_kl
export ACTOR_TOKEN_BUDGET=6144
export EXTRA_HYDRA_ARGS="trainer.stop_file=${FOCUSED_ROOT}/STOP"
bash "${SCRIPT_DIR}/run_screen_container2.sh"
rank_kl_dir="${RUN_ROOT}/qwen3-4b-thinking-2507_rank_kl_h0p0_q0p0_s0"
[[ -f "${rank_kl_dir}/completed.json" ]] || { printf 'Rank-KL paused or incomplete.\n'; exit 1; }
evaluate "${rank_kl_dir}/latest" rank_kl_step100
python -m experiments.compare_evaluations "${EVAL_ROOT}/positive_ce_step100" "${EVAL_ROOT}/rank_kl_step100" --output "${FOCUSED_ROOT}/ce100_vs_rankkl100.json"
python -m experiments.compare_evaluations "${EVAL_ROOT}/rank_jsd_step100" "${EVAL_ROOT}/rank_kl_step100" --output "${FOCUSED_ROOT}/rankjsd100_vs_rankkl100.json"
printf 'Rank-KL training and matched comparisons complete.\n'
