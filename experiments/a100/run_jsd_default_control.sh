#!/usr/bin/env bash
# Matched current-runtime Rank-JSD beta1=0.9 control.
...
print('Previous evaluation verified; starting current-runtime Rank-JSD beta1=0.9.')
CHECK
...
export RUN_ROOT="${FOCUSED_ROOT}/training_jsd_beta1_default"
export LOG_ROOT="${RUN_ROOT}/logs"
export MAX_STEPS=100 TEST_FREQ=25 SAVE_FREQ=10 KEEP_CHECKPOINTS=1 METHODS=rank_jsd
export ACTOR_TOKEN_BUDGET=6144
export EXTRA_HYDRA_ARGS="actor.adam_betas=[0.9,0.999] trainer.experiment_name=qwen3-4b-thinking-2507_rank_jsd_beta1_default_s0 trainer.stop_file=${FOCUSED_ROOT}/STOP"
bash "${SCRIPT_DIR}/run_screen_container2.sh"
rank_kl_dir="${RUN_ROOT}/qwen3-4b-thinking-2507_rank_jsd_h0p0_q0p0_s0"
[[ -f "${rank_kl_dir}/completed.json" ]] || { printf 'Rank-JSD beta1=0.9 paused or incomplete.\n'; exit 1; }
evaluate "${rank_kl_dir}/latest" rank_jsd_beta1_default_step100
python -m experiments.compare_evaluations "${EVAL_ROOT}/positive_ce_step100" "${EVAL_ROOT}/rank_jsd_beta1_default_step100" --output "${FOCUSED_ROOT}/ce100_vs_jsd_beta1_default100.json"
python -m experiments.compare_evaluations "${EVAL_ROOT}/rank_jsd_beta1_zero_step100" "${EVAL_ROOT}/rank_jsd_beta1_default_step100" --output "${FOCUSED_ROOT}/jsd_beta1_zero_vs_default100.json"
printf 'Rank-JSD beta1=0.9 training and matched comparisons complete.\n'
