...
def aggregate_values(
    ...
    if advantages is not None:
        ...
        per_seq_values = tensor.sum(-1) / (
            action_mask.sum(-1) + torch.finfo(tensor.dtype).eps
        )

        # If no sequences pass the filter, return 0 but maintain gradient flow
        if not correct_path.any():
            return per_seq_values.sum() * 0.0  # Keeps gradient graph

        return per_seq_values[correct_path].mean()
    elif avg_level == "token":
