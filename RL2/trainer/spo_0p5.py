...
class SPOTrainer(Trainer):
    ...
    def compute_advantages(self, tensor_dict, cu_seqs, step):
        ...
        elif self.config.adv.estimator == "spo":
            compute_spo_adv(
                tensor_dict,
                cu_seqs,
                self.config.train_data.responses_per_prompt
        ...
        for epoch in range(
            ...
            for data_list in tqdm(
                ...
                if self.config.actor.kl.coef > 0:
                    tensor_dict = self.ref_actor.compute_probs(tensor_dict, step)
                if self.config.adv.estimator == "gae":
                ...
                if self.config.actor.kl.coef > 0 or self.config.actor.update_per_rollout > 1:
                    tensor_dict = self.actor.compute_probs(tensor_dict, step)
                ...
                self.actor.update_0p5(tensor_dict, step)
                if self.config.adv.estimator == "gae":
...
@hydra.main(config_path="config", config_name="spo", version_base=None)
def main(config):
