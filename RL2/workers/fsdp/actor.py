...
from RL2.utils.functions import (
    compute_probs_and_entropy, aggregate_values
)
...
class FSDPActor(FSDPWorker):
    ...
    @time_logger("update_actor")
    def spo_update(self, tensor_dict, step: int):
        if step < self.config.freeze_steps:
            return
        batches = self.scatter_data(tensor_dict, pack_minibatches=True)
        self.load_model_to_device(torch.cuda.current_device())
        ...
        for batch in batches:
            ...
            for minibatch in batch:
                ...
                log1mp = minibatch["log1mp"]
                log_05 = math.log(0.5)

                original_losses = 0.5 * (logps + log1mp) - log_05

                good_losses = torch.where(logps >= log_05, original_losses, 0)
                bad_losses = torch.where(logps <= log_05, original_losses, 0)

                objective = minibatch["advantages"] > 0
                
                losses = torch.where(objective, 
                    minibatch["advantages"] * good_losses, 
                    -minibatch["advantages"] * bad_losses
                    )
                ...
                loss, entropy = aggregate_values(
                    (losses, minibatch["entropy"]),
                    minibatch["action_mask"],
                    self.config.avg_level,
                    total_actions,
                    total_sequences
